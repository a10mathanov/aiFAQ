"""Модуль конфигурации приложения для tenant test_clinic.
Загружает переменные окружения из файла tenants/test_clinic/.env.
"""
...
class Settings(BaseSettings):
    ...
    # By default no proxy. Configure `SOCKS5_PROXY_URL` in tenants/test_clinic/.env if needed.
    ...
    SA_KEY_PATH: str = str(Path(__file__).resolve().parent.parent / "sa-key.json")
...
root_sa = Path(__file__).resolve().parent.parent / "sa-key.json"
if not sa_key_path.exists():
    if root_sa.exists():
        ...
        print(f"❌ ОШИБКА: service account key not найден по пути {sa_key_path} и отсутствует {root_sa}.")
